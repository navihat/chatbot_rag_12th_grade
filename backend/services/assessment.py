...
async def evaluate_assessment_submission(
    ...
    avg_response_time = sum(response_times.values()) / len(response_times) if response_times else 15.0
    ...
    save_chapter_mastery(email, chapter, score, status)
    
    # Save details of each question to chapter_assessment_details
    for i, q in enumerate(questions):
        q_id_str = str(q["id"])
        level = q.get("level", 1)
        resp_time = response_times.get(q_id_str, 15.0)
        save_assessment_detail(email, chapter, level, correctness[i], resp_time)
...
def get_user_mastery(email: str) -> list:
